...
> 发布订阅模型有一个天然的时间差问题：**订阅者晚到了，
> 之前发布的消息它就再也看不到了**。保留消息（retained message）
> 专门解决这个问题。

正常情况下，broker 只把消息转发给**当前已经订阅**的客户端——
如果消息发布在订阅者连接上来之前，这条消息对它来说就永远错过了。
发布时如果带上 **retained 标记**，broker 会把这条消息**额外
存一份，按主题保留最新的一条**；之后任何新订阅这个主题的客户端，
连接完成的瞬间会立刻收到这条保留的消息，而不用等下一次真正
有人发布新消息。

典型用途是“状态类”而不是“事件类”的数据——比如某个设备当前的
在线状态、某个开关当前的开合状态：新上线的监控页面订阅这个主题，
应该立刻拿到“当前状态”，而不是傻等到下一次状态变化才有数据。

Rust 发布一条保留消息（`rumqttc` 的 `publish` 最后一个参数
即是否 retain）：

```rust
client.publish("devices/device-01/status", QoS::AtLeastOnce, true, "online")?;
//                                                             ^^^^ retain = true
```
...
```go
client.Publish("devices/device-01/status", 1, true, "online")
//                                          ^^^^ retain = true
```

保留消息和上一节的**遗嘱消息**经常搭配使用：设备上线时发一条
`retain=true` 的 `"online"`，同时注册一条 `retain=true` 的
`"offline"` 遗嘱——这样不管什么时候有新客户端订阅这个主题，
立刻就能拿到“这个设备现在到底在线还是离线”的最新状态，
不需要额外发一次查询请求。
...
> 从 HTTP/普通 TCP 编程转来的读者，最容易带错的几个假设。

- **误区一：publish 成功返回，就代表所有订阅者都收到了。**
  和 QoS 那一节强调的一样，`publish` 的确认只发生在“客户端到
  broker”这一跳，broker 再往下转发给多个订阅者的过程，
  是否每一个都送达取决于**每个订阅连接各自的 QoS**，不是发布者
  这一次调用能保证的；
- **误区二：主题（topic）需要提前在 broker 上“注册”才能用。**
  MQTT 的 topic 是完全动态的字符串，发布者往一个从没出现过的
  topic 发消息完全合法，broker 不需要任何预先配置——这一点和
  很多消息队列产品要求“先创建 topic/队列”不同，容易先入为主
  地以为 MQTT 也需要；
- **误区三：QoS 数字越大，性能损耗可以忽略。** QoS 2 的四次
  交互（`PUBLISH`→`PUBREC`→`PUBREL`→`PUBCOMP`）比 QoS 0 多了
  好几倍的网络往来，大量高频消息如果都用 QoS 2，broker 和网络
  的压力会明显增加，选择 QoS 应该按“这条消息真的需要多强的
  保证”来决定，不是无脑全用最高档；
- **误区四：清除会话（clean session=true）之后重连，之前的
  订阅关系还在。** 恰恰相反，清除会话意味着 broker **不会**
  保留这个客户端 ID 之前的订阅和未确认消息，重连后需要重新
  `subscribe` 一次——如果希望断线重连后订阅关系自动恢复，
  必须使用非清除会话（`clean_session=false` 或 MQTT 5 的
  会话过期时间）。
...
> MQTT 出问题时，先分清是“连不上 broker”还是“连上了但收不到
> 该收到的消息”，两类问题的排查方向完全不同。

| 现象 | 大概率原因 | 怎么确认 |
| --- | --- | --- |
| `CONNECT` 之后立刻被断开 | 客户端 ID 冲突（另一个客户端用了同样的 ID 已经连着）、认证失败、或协议版本不匹配 | 查看 broker 日志里对应这次连接的断开原因；换一个唯一的 client ID 重试 |
| 订阅成功，但发布的消息完全收不到 | topic 拼写不一致（大小写、多余的 `/`），或用了错误的通配符位置 | 打印实际订阅的 topic 字符串和实际发布的 topic 字符串逐字符对比 |
| 断线重连后，之前订阅的消息不再收到 | 用了清除会话（clean session），订阅关系没有被保留 | 检查连接选项里的 clean session / 会话过期设置 |
| 客户端一段时间后被 broker 判定离线，但客户端进程还在跑 | 保活间隔内既没有业务消息也没有 `PINGREQ`，可能是客户端心跳逻辑卡住了 | 检查客户端是否真的按保活间隔发出了 `PINGREQ`，抓包确认 |
| 新上线的订阅者拿不到设备当前状态 | 发布方发消息时没有加 `retain` 标记 | 检查发布状态类消息时 retain 参数是否设为 true |
...
- **保活间隔（keep alive）** —— 客户端承诺的最长静默时间，超过则
  broker 认为其已掉线。
- **保留消息（retained message）** —— broker 为某个主题额外保存
  的最新一条消息，新订阅者连接后立即收到，不用等下一次发布。
- **清除会话（clean session）** —— 连接选项，决定断线重连后
  broker 是否保留之前的订阅关系和未确认消息。
