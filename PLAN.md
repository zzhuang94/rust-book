...
**规范 5 · 自然段落，少强制折行**
正文以**自然段落**为主：一句写完再开下一句，依赖阅读器自动换行即可。  
**不要**为了「每行 ≤80 字」在行尾加两个空格做强制 `<br>`——那样会把本可连贯的句子拆碎，手机上尤其难看。  
仅在真正需要「视觉上紧挨的两行」（极少）或分点分条时再折；折行时也**绝不**在链接 `[..](..)` 或行内代码 `` `..` `` 中间断开。  
源码仓库里可用适度软换行方便 diff，但**渲染效果应以自然段落为准**。
...
> 2026-07-18 加厚：`docs/os/*` 与 `docs/network/*` 继续扩写保姆级内容；  
> 同时取消「正文强制 ≤80 折行」——改为自然段落（见上文规范 5）。
